...
describe("@state decorator error when Babel plugin not configured", () => {
    it("should throw error when decorator executes at runtime (Stage 3 format)", () => {
        ...
        // Call state decorator with Stage 3 format - should throw error immediately
        expect(() => {
            state(mockContext);
        }).toThrow(/Babel plugin/);
        expect(() => {
            state(mockContext);
        }).toThrow(/testProperty/);
        expect(() => {
            state(mockContext);
        }).toThrow(/@wsxjs\/wsx-vite-plugin/);
    });

    it("should throw error when decorator executes at runtime (legacy format)", () => {
        // Simulate legacy decorator format
        const mockTarget = {};
        const propertyKey = "testProperty";

        // Call state decorator with legacy format - should throw error immediately
        expect(() => {
    ...
    it("should throw error for primitives when Babel plugin not configured", () => {
        const mockContext = {
            kind: "field" as const,
            name: "count",
            addInitializer: jest.fn(),
        ...
        // Call state decorator - should throw error immediately
        expect(() => {
            state(mockContext);
        }).toThrow(/Babel plugin/);
        expect(() => {
    ...
    it("should throw error for objects when Babel plugin not configured", () => {
        const mockContext = {
            kind: "field" as const,
            name: "state",
            addInitializer: jest.fn(),
